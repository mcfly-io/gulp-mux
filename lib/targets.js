...
var askForTargets = function(taskname, opts) {
    ...
    if(!_(opts.target).isEmpty() && !_(opts.target).sortBy().isEqual(_(argv.target).sortBy())) {
        var targetMessage = '(' + chalk.yellow('WARNING') + ') Task \'' + chalk.cyan(taskname) + '\' is overriding command line targets to ' + chalk.yellow(opts.target.toString());
        gutil.log(chalk.green(targetMessage));
        argv.t = argv.target = opts.target;
    }

    if(opts.mode && opts.mode !== argv.mode) {
        var modeMessage = '(' + chalk.yellow('WARNING') + ') Task \'' + chalk.cyan(taskname) + '\' is overriding command line mode to ' + chalk.yellow(opts.mode);
        gutil.log(chalk.green(modeMessage));
 ...
 */
exports.askForMultipleTargets = function(taskname, opts) {
 ...
 * @returns {Object} - The yargs object
 */
exports.askForSingleTarget = function(taskname, opts) {
